...
static const struct argp_option argp_default_options[] =
{
	...
	{"usage",      OPT_USAGE,    0, 0,  "Give a short usage message", 0},
	{"program-name",OPT_PROGNAME,"NAME", OPTION_HIDDEN, "Set the program name", 0},
	{"HANG",       OPT_HANG,    "SECS", OPTION_ARG_OPTIONAL | OPTION_HIDDEN, "Hang for SECS seconds (default 3600)", 0},
	{0}
};
...
parser_finalize (struct parser *parser,
	int err, int arg_ebadkey, int *end_index)
{
	struct group *group;
...
parser_parse_opt (struct parser *parser, int opt, char *val)
{
	...
	if (err == EBADKEY)
		/* At least currently, an option not recognized is an error in the
			...
			with each option.  */
	{
		static const char bad_key_err[] = "(PROGRAM ERROR) Option should have been recognized!?";
		if (group_key == 0)
